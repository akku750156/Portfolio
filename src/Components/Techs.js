...
function Techs() {
  return (
    <div className="py-4 px-4 bg-primary min-h-screen">
        ...
        {skills.map((skill) => (
          <div className="flex justify-center items-center px-2 sm:px-3 md:px-4 lg:px-6 border-2 m-3 border-yellow-300 rounded-xl bg-secondary shadow-3xl">
            {/* <Progress
              ...
              strokeColor="#10B981"
              format={() => (
                <div className="h-16 w-16 m-2 sm:m-3 md:m-4 lg:m-5 sm:h-20 sm:w-20 md:h-24 md:w-24 lg:h-28 lg:w-28 border-0 border-yellow-600 hover:bg-transparent transition-all p-5 lg:p-6 rounded-xl flex flex-col justify-center items-center ">
                  <img src={skill.img} alt="" className="bg-transparent" />
                  <div className="text-xs sm:text-base md:text-xl lg:text-2xl bg-transparent text-white">
                    {skill.name}
                  </div>
                </div>
              )}
